...
$sql = "SELECT * FROM sk_resetLozinke WHERE korisnikID = ? ORDER BY vrijeme DESC LIMIT 1";
$params = [$userID];
$row = fetchSingleResult($con, $sql, $params);

if ($row) {
    $lastRequestTime = strtotime($row['vrijeme']);
    $currentTime = time();
    $diff = $currentTime - $lastRequestTime;
    if ($diff < 1800) {
        echo json_encode([
            'status' => 'error',
            'message' => 'Zahtjev za promjenu lozinke je već poslan. Molimo pričekajte 30 minuta za ponovni pokušaj.'
        ]);
        exit;
    }
}
...
try {
    ...
    $mail->Body    = "
        ...
        <a href='{$resetURL}'>Kliknite ovdje za resetiranje lozinke</a>
