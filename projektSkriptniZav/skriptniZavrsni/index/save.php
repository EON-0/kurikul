...
$realizacije = is_array($realizacije) ? $realizacije : [];
$realizacije_db = fetchMultipleResults($con, "SELECT ID FROM sk_realizacije WHERE AktivnostID = ? AND Obrisano = 0", [$aktivnost_ID]);
...
foreach ($realizacije_db as $realizacija_db) {
    if (!in_array($realizacija_db['ID'], $realizacije_ids)) {
        placeToDataBase($con, "UPDATE sk_realizacije SET Obrisano = 1 WHERE ID = ?", [$realizacija_db['ID']]);
    }
}

foreach ($realizacije as $realizacija) {
    if ($realizacija['ID'] == 0) {
        placeToDataBase($con, "INSERT INTO sk_realizacije (AktivnostID, Realizacija, Obrisano) VALUES (?, ?, 0)", [$aktivnost_ID, $realizacija['realizacija']]);
    } else {
        placeToDataBase($con, "UPDATE sk_realizacije SET Realizacija = ? WHERE ID = ?", [$realizacija['realizacija'], $realizacija['ID']]);
    }
}
// -----------------
// VREDNOVANJA
// -----------------
$vrednovanja = is_array($vrednovanja) ? $vrednovanja : [];
$vrednovanja_db = fetchMultipleResults($con, "SELECT ID FROM sk_vrednovanja WHERE AktivnostID = ? AND Obrisano = 0", [$aktivnost_ID]);
...
foreach ($vrednovanja_db as $vrednovanje_db) {
    if (!in_array($vrednovanje_db['ID'], $vrednovanja_ids)) {
        placeToDataBase($con, "UPDATE sk_vrednovanja SET Obrisano = 1 WHERE ID = ?", [$vrednovanje_db['ID']]);
    }
}
foreach ($vrednovanja as $vrednovanje) {
    if ($vrednovanje['ID'] == 0) {
        // Insert new evaluation
        placeToDataBase($con, "INSERT INTO sk_vrednovanja (AktivnostID, Vrednovanje, Obrisano) VALUES (?, ?, 0)", [$aktivnost_ID, $vrednovanje['vrednovanje']]);
    } else {
        // Update existing evaluation
        placeToDataBase($con, "UPDATE sk_vrednovanja SET Vrednovanje = ? WHERE ID = ?", [$vrednovanje['vrednovanje'], $vrednovanje['ID']]);
    }
}
...
// TROŠKOVNIK
// -----------------
$troskovnik = is_array($troskovnik) ? $troskovnik : [];
$troskovnik_db = fetchMultipleResults($con, "SELECT ID FROM sk_troskovnik WHERE AktivnostID = ? AND Obrisano = 0", [$aktivnost_ID]);
$troskovnik_ids = array_column($troskovnik, 'ID');

foreach ($troskovnik_db as $trosak_db) {
    if (!in_array($trosak_db['ID'], $troskovnik_ids)) {
        placeToDataBase($con, "UPDATE sk_troskovnik SET Obrisano = 1 WHERE ID = ?", [$trosak_db['ID']]);
    }
}
foreach ($troskovnik as $trosak) {
    if ($trosak['ID'] == 0) {
        // Insert new expense
        placeToDataBase($con, "INSERT INTO sk_troskovnik (AktivnostID, Trosak, Obrisano) VALUES (?, ?, 0)", [$aktivnost_ID, $trosak['trosak']]);
    } else {
        // Update existing expense
        placeToDataBase($con, "UPDATE sk_troskovnik SET Trosak = ? WHERE ID = ?", [$trosak['trosak'], $trosak['ID']]);
    }
}
