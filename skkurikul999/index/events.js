...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (kopirajButton) {
        kopirajButton.addEventListener('click', function(event) {
            event.preventDefault();
            getStatic(); //doda danasnji datum, autor postaje trenutni korisnik
            appState.novaAktivnost = true; //kreira se nova aktivnost
            appState.globalID = -1; //neznamo id
            alert("Kopirali se prije odabranu aktivnost.\nZa spremanje nove kopirane aktivnosti pritisnite SAVE");
        });
    }
    //goal button
        // Get elements
...
document.getElementById("aktualna-godina").addEventListener("change", function(event) {
    if (event.target.checked) {
        appState.trenutnaGodina = 1;
    } else {
        appState.trenutnaGodina = -1;
    }
