...
//za zbrisati
function scrolldiv() {
    var elem = document.getElementByText("Markus Pilić");
    elem.scrollIntoView();
}
...
function makeDocument() {
    ...
    $.ajax({
        ...
        data: {
            ...
            realizacije: realizacije,
            troskovnik: troskovnik,
            vrednovanja: vrednovanje  // ensure the key name matches what you use in PHP
        },
        dataType: "JSON",
        success: function (odgovor) {
            // For example, show an alert and insert the HTML into a container:
            alert(odgovor["pravo"]);
            $('#yourContainer').html(odgovor["html"]);
        },
        error: function () {
            alert("Došlo je do pogreške prilikom stvaranja dokumenta");
        }
    });
}
