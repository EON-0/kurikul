...
if (!$pravo || $pravo['imaPravo'] != 1) {
    echo json_encode(["status" => "Nemate pravo uređivanja!"]);
    exit;
}
...
$mozePotvrditi = fetchSingleResult($con, $sql, [$aktivnost_ID]);

if ($mozePotvrditi && $mozePotvrditi['imaPravo'] != 1) {
    echo json_encode(["status" => "Aktivnost se jos odvija."]);
    exit;
}
